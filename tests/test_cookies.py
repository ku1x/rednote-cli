...
from rednote_cli.cookies import (
...
def tmp_config_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("rednote_cli.cookies.get_config_dir", lambda: tmp_path)
    monkeypatch.setattr("rednote_cli.cookies.get_cookie_path", lambda: tmp_path / "cookies.json")
    monkeypatch.setattr("rednote_cli.cookies._TOKEN_CACHE_MEMORY", None)
    monkeypatch.setattr("rednote_cli.cookies._TOKEN_CACHE_PATH", None)
    return tmp_path
...
class TestGetCookies:
    def test_prefers_saved_cookies_by_default(self, monkeypatch):
        monkeypatch.setattr("rednote_cli.cookies.load_saved_cookies", lambda: {"a1": "saved"})
        monkeypatch.setattr(
            "rednote_cli.cookies.extract_browser_cookies",
    ...
    def test_force_refresh_bypasses_saved_cookies(self, monkeypatch):
        ...
        saved = []
        monkeypatch.setattr("rednote_cli.cookies.save_cookies", lambda cookies: saved.append(cookies))
