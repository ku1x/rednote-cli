...
from rednote_cli.cli import cli
from rednote_cli.exceptions import NoCookieError, SessionExpiredError, UnsupportedOperationError
...
class TestCliBasic:
    ...
    def test_status_auto_yaml_when_stdout_is_not_tty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "rednote_cli.commands.auth.run_client_action",
    ...
    def test_read_error_yaml_when_not_logged_in(self, monkeypatch):
        monkeypatch.setenv("OUTPUT", "auto")
        monkeypatch.setattr(
            "rednote_cli.commands._common.get_cookies",
    ...
    def test_status_reports_not_authenticated_when_session_expired(self, monkeypatch):
        ...
        monkeypatch.setattr("rednote_cli.commands.auth.run_client_action", fake_run_client_action)
    ...
    def test_logout_supports_structured_output(self):
        from rednote_cli.commands import auth
    ...
    def test_delete_reports_unsupported_operation(self, monkeypatch):
        monkeypatch.setattr(
            "rednote_cli.commands.creator.run_client_action",
    ...
    def test_comments_rich_output_handles_string_reply_counts(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "rednote_cli.commands.reading.run_client_action",
            lambda ctx, action: {
                "comments": [
                    {
    ...
    def test_search_rich_output_shortens_visible_links(self, monkeypatch):
        monkeypatch.setenv("OUTPUT", "rich")
        monkeypatch.setattr(
            "rednote_cli.commands.reading.handle_command",
    ...
    def test_read_index_resolves_note_context(self, monkeypatch):
        monkeypatch.setattr(
            "rednote_cli.note_refs.get_note_by_index",
            lambda idx: {
                "note_id": "note-abc",
        ...
        monkeypatch.setattr("rednote_cli.commands.reading.handle_command", fake_handle_command)
    ...
    def test_comments_index_resolves_note_context(self, monkeypatch):
        ...
        monkeypatch.setattr("rednote_cli.commands.reading.run_client_action", fake_run_client_action)
    ...
    def test_read_index_not_found_returns_usage_error(self, monkeypatch):
        monkeypatch.setattr("rednote_cli.note_refs.get_note_by_index", lambda idx: None)
    ...
    def test_search_empty_results_clear_previous_index(self, monkeypatch):
        from rednote_cli.note_refs import save_index_from_items

        saved = []
        monkeypatch.setattr("rednote_cli.note_refs.save_note_index", lambda items: saved.append(items))
    ...
    def test_short_index_resolves_for_note_actions(self, monkeypatch, command, extra_args, method_name):
        ...
        monkeypatch.setattr("rednote_cli.commands.interactions.handle_command", fake_handle_command)
    ...
    def test_favorites_saves_index_entries(self, monkeypatch):
        saved = []
        monkeypatch.setattr("rednote_cli.note_refs.save_note_index", lambda items: saved.append(items))
        monkeypatch.setattr("rednote_cli.commands.social._resolve_user_id", lambda ctx, user_id: "user-1")
        ...
        monkeypatch.setattr("rednote_cli.commands.social.handle_command", fake_handle_command)
    ...
    def test_my_notes_saves_index_entries(self, monkeypatch):
        ...
        monkeypatch.setattr("rednote_cli.commands.creator.handle_command", fake_handle_command)
