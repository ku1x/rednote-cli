...
from rednote_cli.command_normalizers import normalize_xhs_user_payload
from rednote_cli.exceptions import XhsApiError
from rednote_cli.qr_login import BrowserQrLoginUnavailable, _normalize_browser_cookies, qrcode_login
...
def test_qrcode_login_completes_after_confirmation_and_saves_real_session(monkeypatch):
    ...
    monkeypatch.setattr("rednote_cli.qr_login.XhsClient", _FakeQrClient)
    monkeypatch.setattr("rednote_cli.qr_login._generate_a1", lambda: "a1-fixed")
    monkeypatch.setattr("rednote_cli.qr_login._generate_webid", lambda: "webid-fixed")
    monkeypatch.setattr("rednote_cli.qr_login._display_qr_in_terminal", lambda data: True)
    monkeypatch.setattr("rednote_cli.qr_login.time.sleep", lambda seconds: None)
    monkeypatch.setattr("rednote_cli.qr_login.save_cookies", lambda cookies: saved.append(cookies))
...
def test_qrcode_login_accepts_confirmed_user_from_self_info_fallback(monkeypatch):
    ...
    monkeypatch.setattr("rednote_cli.qr_login.XhsClient", _SelfInfoFallbackQrClient)
    monkeypatch.setattr("rednote_cli.qr_login._generate_a1", lambda: "a1-fixed")
    monkeypatch.setattr("rednote_cli.qr_login._generate_webid", lambda: "webid-fixed")
    monkeypatch.setattr("rednote_cli.qr_login._display_qr_in_terminal", lambda data: True)
    monkeypatch.setattr("rednote_cli.qr_login.time.sleep", lambda seconds: None)
...
def test_qrcode_login_rejects_mismatched_confirmed_user(monkeypatch):
    monkeypatch.setattr("rednote_cli.qr_login.XhsClient", _MismatchQrClient)
    monkeypatch.setattr("rednote_cli.qr_login._generate_a1", lambda: "a1-fixed")
    monkeypatch.setattr("rednote_cli.qr_login._generate_webid", lambda: "webid-fixed")
    monkeypatch.setattr("rednote_cli.qr_login._display_qr_in_terminal", lambda data: True)
    monkeypatch.setattr("rednote_cli.qr_login.time.sleep", lambda seconds: None)
    monkeypatch.setattr("rednote_cli.qr_login.save_cookies", lambda cookies: None)
...
def test_qrcode_login_prefers_browser_assisted_backend(monkeypatch):
    ...
    monkeypatch.setattr(
        "rednote_cli.qr_login._browser_assisted_qrcode_login",
        lambda **kwargs: {
            "a1": "a1-browser",
            "webId": "webid-browser",
...
def test_qrcode_login_falls_back_when_browser_backend_unavailable(monkeypatch):
    ...
    monkeypatch.setattr(
        "rednote_cli.qr_login._http_qrcode_login",
        lambda **kwargs: {
            "a1": "a1-http",
            "webId": "webid-http",
